...
import controle.DAO.CarrinhoJpaController;
...
public class ServletCarrinhoController extends HttpServlet {
    ...
    private void incluir() {
        ...
        try {
            ...
            Logger.getLogger(ServletCarrinhoController.class.getName()).log(Level.SEVERE, null, ex);
        } catch (Exception ex) {
            Logger.getLogger(ServletCarrinhoController.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    private void listar() {
        EntityManagerFactory emf = Persistence.createEntityManagerFactory("EcommerceAllan_UbiraciPU");
        CarrinhoJpaController carrinhoDAO = new CarrinhoJpaController(emf);
        for (Carrinho carrinho : carrinhoDAO.findCarrinhoEntities()) {
    ...
    private void alterar(Long id)  {
        EntityManagerFactory emf = Persistence.createEntityManagerFactory("EcommerceAllan_UbiraciPU");
        CarrinhoJpaController carrinhoDAO = new CarrinhoJpaController(emf);
        Carrinho carrinho = carrinhoDAO.findCarrinho(id);
        carrinho.setDataCompra(new Date());
        try {
            carrinhoDAO.edit(carrinho);
        } catch (RollbackFailureException ex) {
            Logger.getLogger(ServletCarrinhoController.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
